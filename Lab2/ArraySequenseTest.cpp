//
// Created by Богдан Брыксин on 27.04.2022.
//

#include"ArraySequenseTest.h"
#include <iostream>
...
void ArraySequence_Get_first_test()
{
    int items[5]={123,312,412,45,12};
    ArraySequence <int> *array=new ArraySequence <int>(items,5);
    assert(array->GetFirst()==items[0]);
}
void ArraySequence_Get_last_test()
{
    int items[5]={123,312,412,45,12};
    ArraySequence <int> *array=new ArraySequence <int>(items,5);
    assert(array->GetLast()==items[4]);
}
...
void ArraySequence_tests()
{
    ...
    ArraySequence_Get_first_test();
    ArraySequence_Get_last_test();
